...
| 항목 | 방향 |
|------|------|
| 분위기 | 검정과 금. 금에 주황을 섞어 데웠다 |
| 레퍼런스 | Nike Training Club, Whoop |
| 핵심 색상 | 주황이 도는 금 (#eeb77d) |
| 테마 | 다크 고정 (라이트 모드 없음) |
...
### 포인트
- `--accent`: #eeb77d (주황을 30% 섞은 금)
- `--accent-hover`: #f7c499 (밝은 금)
- `--accent-low`: #d29a5f (어두운 금 — 그라디언트의 아래쪽)
- `--accent-dim`: #241a11 (금빛이 도는 어두운 바탕)
- `--on-accent`: #12100c — **금을 통째로 칠했을 때 그 위에 얹는 글자.**
  금이 밝아진 뒤로 흰 글자는 아예 안 보인다 (1.8:1)
...
`src/data/chartColors.js` — **앱에서 색을 직접 적는 유일한 자리다.**

recharts 는 색을 SVG 속성으로 내보내고(`<text fill="…">` · `<path stroke="…">`),
속성 안에서는 `var()` 가 치환되지 않는다. 그래프에 토큰을 넘기면 값이 통째로
무시되면서 축 글씨는 검정, 선은 안 그려진다 — 에러 없이 조용히.

그래서 토큰과 같은 값을 손으로 맞춰 두고, `npm run check` 가 아홉 쌍을 비교한다.
색을 고칠 때는 `globals.css` 와 `chartColors.js` **둘 다** 고친다.
