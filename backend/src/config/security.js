...
const RATE_LIMITS = {
  ...
  // 한 번 돌 때 여덟 번 넣는데, 20 이면 두 번 돌리고 나서 **검사가 제 제한에 걸렸다** —
  // 그때 화면(과 검사)에 뜨는 말은 「429」라서 파일이 잘못된 것처럼 읽힌다.
  // 사람이 제 기록을 되돌리는 일은 시간당 60 이면 넉넉하고(되돌리기는 대개 세 번이다),
  // 막으려는 것은 「수만 줄을 쉼 없이 밀어 넣는 것」이지 되돌리는 사람이 아니다
  importCsv:   { windowMs: 60 * 60 * 1000,     max: 60  },
};
