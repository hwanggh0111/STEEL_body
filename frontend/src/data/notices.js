...
// 공지 유형 — 앱 어디서든 이 목록 하나만 본다.
// 예전에는 같은 배열이 NoticePage(작성·수정·필터) · NoticeAdmin · AiNoticeWriter 에
// 따로 박혀 있어서, AI 작성 유형만 4종(점검 포함)으로 어긋난 채 남아 있었다.
export const NOTICE_TYPES = ['공지', '긴급공지', '업데이트', '신기능', '이벤트'];

// AI 작성이 백엔드에 보내는 키 ↔ 화면에 보이는 유형. 위 목록과 1:1 로 맞춘다.
export const AI_TYPES = [
  { key: 'notice',     label: '공지' },
  { key: 'urgent',     label: '긴급공지' },
  { key: 'update',     label: '업데이트' },
  { key: 'newfeature', label: '신기능' },
  { key: 'event',      label: '이벤트' },
];

export const aiLabel = (key) => (AI_TYPES.find(t => t.key === key) || AI_TYPES[0]).label;
