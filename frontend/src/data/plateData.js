...
export const PLATES = [
  ...
  //
  // 확률은 일부러 잔인하게 잡았다 — 슈퍼울트라보다 250배 희귀하다.
  // 운영 weight 0.002 기준 (총 가중치 100.502):
  //   한 원판이 무한일 확률   0.00199%
  //   한 판(원판 120개)에 뜰 확률  약 0.24%  → 419판에 한 번
  //   하루 5판이면 약 84일(약 3개월)에 한 번
  //
  // 이 값 하나가 수급을 좌우한다. 참고로 주울 때 평균 기대값은:
  //   0.01  → 7.20 (+16%)   약 17일에 한 번
  //   0.002 → 6.41 (+3%)    약 84일에 한 번   ← 지금
  //   0.001 → 6.31 (+2%)    약 168일에 한 번
  //
  ...
  // 바닥 정리에서 보호되고, 9초를 버티고, 멀리서도 보이게 크게 빛난다.
  {
    ...
    weight: import.meta.env.DEV ? 8 : 0.002,
  ...
  //
  // 운영 weight 0.0005 기준 (총 가중치 100.5025):
  //   한 원판이 울트라 무한일 확률  0.0000498%
  //   한 판(원판 120개)에 뜰 확률   약 0.0597%  → 1,675판에 한 번
  //   하루 5판이면 약 335일 — 대략 1년에 한 번
  //
  ...
  {
    kg: 9999, name: '울트라 무한', display: '∞', value: 99999,
    r: 0.130, speed: 1.15, lethal: true,
    weight: import.meta.env.DEV ? 8 : 0.0005,
