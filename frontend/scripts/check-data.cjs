...
// 9/4 저녁에 둘 늘었다 — 서랍의 **「앱 소개」**(고객센터에서 갈라 나왔다)와
// **「커뮤니티」**(이 앱에서 남에게 보이는 첫 글이다)
...
// 2026-09-19 (8차) 에 **하나 줄었다** — 아래 탭바에서 **「기구」**를 걷었다(`wrench`).
// 7차에 늘린 지 이틀 만이다. 그때 주석에 「탭을 늘리는 것은 이 앱이 계속 거절해 온
// 일이다」라고 적어놓고 늘렸는데, 기구 세팅은 **하루의 축이 아니라 「운동」의 곁다리
// 질문**이었다. 화면(`/gym`)은 그대로고, 「운동」의 기구 카드에서 들어간다
ok('여섯 자리가 아이콘 열일곱 개를 쓴다', used.length, 17);
